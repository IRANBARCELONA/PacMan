...
public class PacMan extends JPanel implements ActionListener, KeyListener {
    ...
    class Block {
        ...
        Block(Image image, int x, int y, int width, int height, char name) {
            this.image = image;
            this.x = x;
            this.y = y;
            this.width = width;
        ...
        for (int r = 0; r < rowCount; r++) {
            for (int c = 0; c < columnCount; c++) {
                ...
                if (tileMapChar == 'X') { //wall phase 3
                    Block wall = new Block(wallph3Image, x, y, tileSize, tileSize);
                    walls.add(wall);
                }
                else if (tileMapChar == 'g') { //wallRP
                    Block wall = new Block(wallRPImage, x, y, tileSize, tileSize );
                    ...
                    Block lightGhost = new Block(lightGhostGImage, x, y, tileSize, tileSize, 'G');
                    lightGhosts.add(lightGhost);
                }
                else if (tileMapChar == 'o') { //light orange ghost
                    Block lightGhost = new Block(lightGhostOImage, x, y, tileSize, tileSize, 'O');
                    lightGhosts.add(lightGhost);
                }
                else if (tileMapChar == 'Y') { //pink ghost
                    Block lightGhost = new Block(lightGhostYImage, x, y, tileSize, tileSize, 'Y');
                    lightGhosts.add(lightGhost);
                }
                else if (tileMapChar == 'R') { //red ghost
                    Block lightGhost = new Block(lightGhostRImage, x, y, tileSize, tileSize, 'R');
                    lightGhosts.add(lightGhost);
                }
                else if (tileMapChar == 'P') { //pacman
                    pacman = new Block(pacmanRightImage, x, y, tileSize, tileSize);
    ...
    public void checkCollision() {
        ...
        for (Block food : foods) {
            if (collision(pacman, food) && lightGhosts.isEmpty()) {
                if(foodEatingSound) {
    ...
    public void lightGhost(){
        ...
        for (Block lightGhost : lightGhosts) {
            ...
            if (collision(lightGhost, pacman)) {
                if(lightGhost.name == 'G')
                    lightGhosts.removeIf(g -> g.name == 'G');
                else if(lightGhost.name == 'Y')
                    lightGhosts.removeIf(g -> g.name == 'Y');
                else if(lightGhost.name == 'R')
                    lightGhosts.removeIf(g -> g.name == 'R');
                else if(lightGhost.name == 'O')
                    lightGhosts.removeIf(g -> g.name == 'O');
                score += 800;
            }
